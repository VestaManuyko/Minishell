 ...
 */
static int	is_valid_path(char *exec)
{
	int			ret;
	struct stat	info;
 ...
 */
static int	check_exec_withpath(char *path, char *slash, void **exec)
{
	char	*full_path;
 ...
 * - 0 if not found.
 */
int	is_executable(t_arr *env, void **exec)
{
	int		ret;
	int		i;
	char	**paths;
	...
	while (paths[i] != NULL)
	{
		ret = check_exec_withpath(paths[i], slash, exec);
		if (ret != 2)
